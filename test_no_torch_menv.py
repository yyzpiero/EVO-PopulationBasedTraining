...
from typing import Any, Callable, List, Optional, Sequence, Tuple, Type, Union, OrderedDict
from stable_baselines3.common.monitor import Monitor
from gym_minigrid.wrappers import *

def main():
    env_id = "nasim:Pocp2Gen-v1"
    seed = 143
    num_envs= 25
    #cuda = True

    #device = torch.device("cuda" if torch.cuda.is_available() and cuda else "cpu")
    envs = make_vec_envs(env_id, seed, num_envs, sub_proc=True)
    ...
    for i in range(10):
        ...
        for step in range(128):
            ...
            action = np.random.randint(low=0, high=action_space_size, size=(envs.num_envs), dtype=int)
            ob, reward, dones, infos = envs.step(action)
            num_sample += 1

        time_elapsed = time.time()-since
        sps.append(global_step /time_elapsed)
